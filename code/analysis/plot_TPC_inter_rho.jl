...
for index in 1: 990
    next!(progress)
    @load "../results/inter_ij_0_rho/fit_allij_0_rho1_$(index).jld2" fitted
    i = 10*(index-1) + 10
...
CSV.write("../results/αij_fitted_rho1.csv", fitted_all, writeheader=false)
...
fitted_ii_0 = CSV.read("../results/αii_fitted_rho1.csv", DataFrame, header=false)
fitted_ij_0 = CSV.read("../results/αij_fitted_rho1.csv", DataFrame, header=false)
# fitted = vcat(fitted_ii, fitted_ij)
df_names = ["B0","E","Th","Ed","AIC","r2"]
fitted_ii_0 = DataFrame(fitted_ii_0, df_names)
fitted_ij_0 = DataFrame(fitted_ij_0, df_names)

path_ii = ["../results/αii_fitted_rho1.csv"]
path_ij = ["../results/αij_fitted_rho1.csv"]
