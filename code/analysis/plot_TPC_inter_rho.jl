...
#     # @load "../results/inter_ij0_rho4_new/fit_allij_0_rho4_$(index).jld2" fitted
#     @load "../results/20250911/inter_ij_05/fit_allij_05_rho4_$(index).jld2" fitted
#     i = 10*(index-1) + 10
...
# CSV.write("../results/20250911/inter_ij_05/αij_fitted_rho4.csv", fitted_all, writeheader=false)
...
####################### Data for plots ##############
### rho1
fitted_ii_0_rho1 = CSV.read("../results/αii_fitted_rho1.csv", DataFrame, header=false)
fitted_ij_0_rho1 = CSV.read("../results/αij_fitted_rho1.csv", DataFrame, header=false)
# fitted = vcat(fitted_ii, fitted_ij)
df_names = ["B0","E","Th","Ed","AIC","r2"]
fitted_ii_0_rho1 = DataFrame(fitted_ii_0_rho1, df_names); 
fitted_ij_0_rho1 = DataFrame(fitted_ij_0_rho1, df_names); 

@load "../data/1com0_rho1.jld2" all_ℵii all_ℵij # all_up_ℵij all_low_ℵij all_ℵij_sum all_D_ℵij all_ℵii_sur all_ℵij_sur all_up_ℵij_sur all_low_ℵij_sur all_ℵij_sum_sur all_D_ℵij_sur;
all_ℵii_0_rho1 = all_ℵii; all_ℵij_0_rho1 = all_ℵij
path_ii_rho1 = ["../results/αii_fitted_rho1.csv"]
path_ij_rho1 = ["../results/αij_fitted_rho1.csv"]
fitted_0ii_rho1 = filter_fitted(path_ii_rho1[1], p_0)
fitted_0ij_rho1 = filter_fitted(path_ij_rho1[1], p_0)
fitted_0_rho1 = vcat(fitted_0ii_rho1[:,1:7], fitted_0ij_rho1)
...
for i in 1:25
    ...
    hidedecorations!(ax1)
    if i <= 5
        nii_0_rho1 = Int.(range(5*(n-1)+1,5*(n-1)+5,5)[i])
        params_ii_0_rho1 = fitted_ii_0_rho1[Int(nii_0_rho1),1:4]
        αii_0_rho1 = [all_ℵii_0_rho1[t][nii_0_rho1] for t in 1:num_temps]
        pred_ii_0_rho1 = abs.(temp_SS(temp, params_ii_0_rho1))

        # text!(0.0, maximum(abs.(αii)), text = "αᵢᵢ", align = (:left, :top),fontsize = 15)
        scatter!(ax1, Temp_rich, abs.(αii_0_rho1), color = "#5676A5", alpha = 0.9)
        lines!(ax1, Temp_rich, pred_ii_0_rho1, color = ("#FA8328", 0.7), linewidth = 5)
    else 
        nij_0_rho1 = Int.(range(20*(n-1)+1,20*(n-1)+20,20)[i-5])
        params_ij_0_rho1 = fitted_ij_0_rho1[Int(nij_0_rho1),1:4]
        αij_0_rho1 = [all_ℵij_0_rho1[t][nij_0_rho1] for t in 1:num_temps]
        pred_ij_0_rho1 = abs.(temp_SS(temp, params_ij_0_rho1))
        # text!(0.0, maximum(abs.(αij)), text = "αᵢⱼ", align = (:left, :top) ,fontsize = 15)
        scatter!(ax1, Temp_rich, abs.(αij_0_rho1), color = "#5676A5", alpha = 0.9)
        lines!(ax1, Temp_rich, pred_ij_0_rho1, color = ("#015845", 0.7), linewidth = 5)
    end 
end 
...
density!(ax2, fitted_0_rho1.E, label = "Eα", color = ("#C25E8B", 0.8), strokewidth = 3, strokecolor = "#C25E8B")
lines!(ax2, [median(fitted_0_rho1.E), median(fitted_0_rho1.E)], [0, 1.6], linestyle = :dash, color = ("#601210", 1.0), linewidth = 5)
...
density!(ax3, fitted_0_rho1.Th .- 273.15, label = "Tpα", color = ("#5676A5", 0.8), strokewidth = 3, strokecolor = "#5676A5")
lines!(ax3, [median(fitted_0_rho1.Th)- 273.15, median(fitted_0_rho1.Th)- 273.15], [0, 0.12], linestyle = :dash, color = ("#0758AE", 0.9), linewidth = 5)
...
for i in 1:25
    ...
    if i <= 5
        ...
        # text!(0.0, maximum(abs.(αii)), text = "αᵢᵢ", align = (:left, :top),fontsize = 15)
        scatter!(ax4, Temp_rich, abs.(αii_0_rho2), color = "#5676A5", alpha = 0.9)
        lines!(ax4, Temp_rich, pred_ii_0_rho2, color = ("#FA8328", 0.7), linewidth = 5)
    else 
        ...
        # text!(0.0, maximum(abs.(αij)), text = "αᵢⱼ", align = (:left, :top) ,fontsize = 15)
        scatter!(ax4, Temp_rich, abs.(αij_0_rho2), color = "#5676A5", alpha = 0.9)
        lines!(ax4, Temp_rich, pred_ij_0_rho2, color = ("#015845", 0.7), linewidth = 5)
    end 
end 
...
density!(ax5, fitted_0_rho2.E, label = "Eα", color = ("#C25E8B", 0.8), strokewidth = 3, strokecolor = "#C25E8B")
lines!(ax5, [median(fitted_0_rho2.E), median(fitted_0_rho2.E)], [0, 1.6], linestyle = :dash, color = ("#601210", 1.0), linewidth = 5)
lines!(ax5, [median(fitted_0_rho2.E), 3.5],[0.6, 0.6], linestyle = :dot, color = ("#601210", 0.9), linewidth = 3)
text!(ax5, 3.5, 0.6, text = "$(round(median(fitted_0_rho2.E) ,digits = 2)) ev", align = (:left, :center), fontsize = 20, color = "#601210")
...
density!(ax6, fitted_0_rho2.Th .- 273.15, label = "Tpα", color = ("#5676A5", 0.8), strokewidth = 3, strokecolor = "#5676A5")
lines!(ax6, [median(fitted_0_rho2.Th)- 273.15, median(fitted_0_rho2.Th)- 273.15], [0, 0.12], linestyle = :dash, color = ("#0758AE", 0.9), linewidth = 5)
lines!(ax6, [20, median(fitted_0_rho2.Th)- 273.15],[0.095, 0.095], linestyle = :dot, color = ("#0758AE", 0.9), linewidth = 3)
text!(ax6, 20, 0.095, text = "$(round(median(fitted_0_rho2.Th)- 273.15 ,digits = 2)) °C", align = (:right, :center), fontsize = 20, color = "#0758AE")
...
for i in 1:25
    ...
    if i <= 5
        ...
        # text!(0.0, maximum(abs.(αii)), text = "αᵢᵢ", align = (:left, :top),fontsize = 15)
        scatter!(ax7, Temp_rich, abs.(αii_0_rho3), color = "#5676A5", alpha = 0.9)
        lines!(ax7, Temp_rich, pred_ii_0_rho3, color = ("#FA8328", 0.7), linewidth = 5)
    else 
        ...
        # text!(0.0, maximum(abs.(αij)), text = "αᵢⱼ", align = (:left, :top) ,fontsize = 15)
        scatter!(ax7, Temp_rich, abs.(αij_0_rho3), color = "#5676A5", alpha = 0.9)
        lines!(ax7, Temp_rich, pred_ij_0_rho3, color = ("#015845", 0.7), linewidth = 5)
    end 
end 
...
density!(ax8, fitted_0_rho3.E, label = "Eα", color = ("#C25E8B", 0.8), strokewidth = 3, strokecolor = "#C25E8B")
lines!(ax8, [median(fitted_0_rho3.E), median(fitted_0_rho3.E)], [0, 1.6], linestyle = :dash, color = ("#601210", 1.0), linewidth = 5)
lines!(ax8, [median(fitted_0_rho3.E), 3.5],[0.6, 0.6], linestyle = :dot, color = ("#601210", 0.9), linewidth = 3)
text!(ax8, 3.5, 0.6, text = "$(round(median(fitted_0_rho3.E) ,digits = 2)) ev", align = (:left, :center), fontsize = 20, color = "#601210")
...
density!(ax9, fitted_0_rho3.Th .- 273.15, label = "Tpα", color = ("#5676A5", 0.8), strokewidth = 3, strokecolor = "#5676A5")
lines!(ax9, [median(fitted_0_rho3.Th)- 273.15, median(fitted_0_rho3.Th)- 273.15], [0, 0.12], linestyle = :dash, color = ("#0758AE", 0.9), linewidth = 5)
lines!(ax9, [20, median(fitted_0_rho3.Th)- 273.15],[0.095, 0.095], linestyle = :dot, color = ("#0758AE", 0.9), linewidth = 3)
text!(ax9, 20, 0.095, text = "$(round(median(fitted_0_rho3.Th)- 273.15 ,digits = 2)) °C", align = (:right, :center), fontsize = 20, color = "#0758AE")
...
#         # text!(0.0, maximum(abs.(αii)), text = "αᵢᵢ", align = (:left, :top),fontsize = 15)
#         scatter!(ax10, Temp_rich, abs.(αii_0_rho4), color = "#5676A5", alpha = 0.9)
#         lines!(ax10, Temp_rich, pred_ii_0_rho4, color = ("#FA8328", 0.7), linewidth = 5)
#     else 
...
#         # text!(0.0, maximum(abs.(αij)), text = "αᵢⱼ", align = (:left, :top) ,fontsize = 15)
#         scatter!(ax10, Temp_rich, abs.(αij_0_rho4), color = "#5676A5", alpha = 0.9)
#         lines!(ax10, Temp_rich, pred_ij_0_rho4, color = ("#015845", 0.7), linewidth = 5)
#     end 
# end 
...
# density!(ax11, fitted_0_rho4.E, label = "Eα", color = ("#C25E8B", 0.8), strokewidth = 3, strokecolor = "#C25E8B")
# lines!(ax11, [median(fitted_0_rho4.E), median(fitted_0_rho4.E)], [0, 1.6], linestyle = :dash, color = ("#601210", 1.0), linewidth = 5)
# lines!(ax11, [median(fitted_0_rho4.E), 3.5],[0.6, 0.6], linestyle = :dot, color = ("#601210", 0.9), linewidth = 3)
# text!(ax11, 3.5, 0.6, text = "$(round(median(fitted_0_rho4.E) ,digits = 2)) ev", align = (:left, :center), fontsize = 20, color = "#601210")
...
# density!(ax12, fitted_0_rho4.Th .- 273.15, label = "Tpα", color = ("#5676A5", 0.8), strokewidth = 3, strokecolor = "#5676A5")
# lines!(ax12, [median(fitted_0_rho4.Th)- 273.15, median(fitted_0_rho4.Th)- 273.15], [0, 0.8], linestyle = :dash, color = ("#0758AE", 0.9), linewidth = 5)
# lines!(ax12, [25, median(fitted_0_rho4.Th)- 273.15],[0.7, 0.7], linestyle = :dot, color = ("#0758AE", 0.9), linewidth = 3)
# text!(ax12, 25, 0.7, text = "$(round(median(fitted_0_rho4.Th)- 273.15 ,digits = 2)) °C", align = (:right, :center), fontsize = 20, color = "#0758AE")
...
save("../results/TPCα_rho_v2.pdf", f) 
