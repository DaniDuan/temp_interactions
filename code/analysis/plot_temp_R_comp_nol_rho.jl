...
@load "../data/summary_stability_L0_rho.jld2" all_leading_collect_1 all_leading_collect_2 all_leading_collect_3 all_leading_collect_4 dℵij_collect_1 dℵij_collect_2 dℵij_collect_3 dℵij_collect_4

sta_1 = [sum(real.(all_leading_collect_1[t]) .< 0)/998 for t in 1:num_temps]
...
f
# save("../results/ratio_sta_nol_rho.pdf", f) 
...
# Label(f[1,0], "     Without Leakage     ", fontsize = 50, rotation = pi/2)
ax1 = Axis(f[1,1], title =  "Without Leakage", xlabel = "Temperature (°C)", ylabel = "Effective Resource Competition\n(αⱼᵢ/αᵢᵢ for j = 1,...,N)", xlabelsize = 45, ylabelsize = 45, ygridvisible = true, xgridvisible = true)
ax2 = Axis(f[1,1], ylabel = "p(Stability)", yaxisposition = :right, yticklabelalign = (:left, :center), xlabelsize = 45, ylabelsize = 45, ygridvisible = false, xgridvisible = false, xticklabelsvisible = false, xlabelvisible = false)
hidespines!(ax2)
ylims!(ax2, 0.9, 1.02)
# hidedecorations!(ax1, grid = false, ticks = true, ticklabels = true)
...
f
save("../results/ratio_sta_nol_rho_v2.pdf", f) 
