...
for i in range(0, stop = 30, length = 31)
    ...
    # N = 1; M= 1
    # Random.seed!(1)
    p = generate_params(N, M; f_u=F_u, f_m=F_m, f_ρ=F_ρ, f_ω=F_ω, L=L, T=T, ρ_t=ρ_t, Tr=Tr, Ed=Ed, niche = niche, input_type = input_type[3])
...
@save "../data/20250814/p0_test_input/Eff_iters0_ρ4_$(index).jld2" rich all_sur all_ℵii all_ℵij all_ℵij_d all_uℵij all_lℵij all_r all_u all_m all_Eu all_Em all_Tpu all_Tpm all_Rrela all_Crela all_R all_C 
