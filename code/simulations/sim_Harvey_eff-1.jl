...
all_ℵ = Vector{Vector{Float64}}(); all_ℵii = Vector{Vector{Float64}}(); all_ℵij = Vector{Vector{Union{Float64, Missing}}}(); all_ℵij_d = Vector{Vector{Union{Float64, Missing}}}();
...
all_u =  Vector{Vector{Float64}}(); all_m =  Vector{Vector{Float64}}(); RO = Vector{Vector{Union{Float64, Missing}}}(); ulO = Vector{Vector{Union{Float64, Missing}}}(); Rul = Vector{Vector{Union{Float64, Missing}}}(); all_UDLD = Vector{Vector{Union{Float64, Missing}}}();
...
@time for i in range(0, stop = 30, length = 31)
    ...
    if N_sur > 1
        ...
        ℵij = [sur_ℵ[i, j] for i in 1:N_sur for j in 1:N_sur if i != j]
        ℵij_d = [sur_ℵ[i, j]/diag(sur_ℵ)[i] for i in 1:N_sur for j in 1:N_sur if i != j]
        UDLD = [sur_ℵ[i, j]/sur_ℵ[j, i] for i in 1:N_sur for j in 1:N_sur if j != i]
        push!(all_ℵ, vec(sur_ℵ)); push!(all_ℵii, ℵii); push!(all_ℵij, ℵij); push!(all_ℵij_d, ℵij_d); push!(all_r, sur_r);
        push!(all_u, u); push!(all_m, m); push!(RO, R_over); push!(ulO, ul_over); push!(Rul, Rul_over); push!(all_UDLD, UDLD);
        ...
        push!(all_ℵ, sur_ℵ); push!(all_ℵii, ℵii); push!(all_ℵij, missing); push!(all_ℵij_d, missing); push!(all_r, sur_r);
        push!(all_u, u); push!(all_m, m); push!(RO, missing); push!(ulO, missing); push!(Rul, missing); push!(all_UDLD, missing);
        push!(all_Eu, Eu); push!(all_Em, Em); push!(all_Eu_sur, Eu_sur); push!(all_Em_sur, Em_sur);
        push!(all_Tpu, Tpu); push!(all_Tpm, Tpm); push!(all_Tpu_sur, Tpu_sur); push!(all_Tpm_sur, Tpm_sur);
        push!(all_leading, leading); push!(all_diag, jac_diag); push!(radi, jac_off); push!(diag_dominance, diag_dom)
    end
...
@save "../data/20240807/new_p-1/Eff_iters-1_$(index).jld2" all_ℵ all_ℵii all_ℵij all_ℵij_d all_r  all_u all_m RO ulO Rul all_UDLD all_Eu all_Em all_Eu_sur all_Em_sur all_Tpu all_Tpm all_Tpu_sur all_Tpm_sur all_leading all_diag radi diag_dominance 

# @load "../data/Eff_iter/Eff_iters1.jld2" all_ℵ all_ℵii all_ℵij all_ℵij_d all_r  all_u all_m RO ulO Rul all_UDLD all_Eu all_Em all_Eu_sur all_Em_sur all_Tpu all_Tpm all_Tpu_sur all_Tpm_sur all_leading all_diag radi diag_dominance 
