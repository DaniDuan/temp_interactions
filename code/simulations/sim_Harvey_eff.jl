...
@time for i in range(0, stop = 30, length = 31)
    ...
    ul_over = 1 .- [bray_curtis_dissimilarity(ul[i,:], u_all[j,:]) for i in 1:N for j in 1:N if j != i]
    ...
    if N_s > 1
        ...
        ul_over_sur = 1 .- [bray_curtis_dissimilarity(ul[i,:], u_t[j,:]) for i in 1:N_s for j in 1:N_s if j != i]
...
@save "../data/20240815/p_re/Eff_iters_re_$(index).jld2" all_ℵii all_ℵij all_ℵij_d all_uℵij all_lℵij all_ℵii_sur all_ℵij_sur all_ℵij_d_sur all_uℵij_sur all_lℵij_sur all_r all_u all_m RO ulO Rul RO_sur ulO_sur Rul_sur all_Eu all_Em all_Eu_sur all_Em_sur all_Tpu all_Tpm all_Tpu_sur all_Tpm_sur all_leading all_diag radi diag_dominance all_Rrela all_Crela
