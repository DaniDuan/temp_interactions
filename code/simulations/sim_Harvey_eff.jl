...
for i in range(0, stop = 30, length = 31)
    ...
    if N_s > 1
        ...
        push!(all_u, vec(u)); push!(all_m, m); push!(RO, R_over); push!(ulO, ul_over); push!(Rul, Rul_over);
        ...
        push!(all_u, vec(u)); push!(all_m, m); push!(RO, [missing]); push!(ulO, [missing]); push!(Rul, [missing]);
