...
all_ℵ = Vector{Vector{Float64}}(); all_ℵii = Vector{Vector{Float64}}(); all_ℵij = Vector{Vector{Union{Float64, Missing}}}(); all_r = Vector{Vector{Float64}}(); 
...
@time for i in range(0, stop = 30, length = 31)
    ...
    if N_sur > 1
        ...
        push!(all_ℵ, vec(sur_ℵ)); push!(all_ℵii, ℵii); push!(all_ℵij, ℵij); push!(all_r, sur_r);
        ...
        push!(all_ℵ, sur_ℵ); push!(all_ℵii, ℵii); push!(all_ℵij, missing); push!(all_r, sur_r);
...
@save "../data/Eff_iters$(index).jld2" all_ℵ all_ℵii all_ℵij all_r  all_u all_m RO ulO Rul all_UDLD all_Eu all_Em all_Eu_sur all_Em_sur all_Tpu all_Tpm all_Tpu_sur all_Tpm_sur all_leading all_diag radi diag_dominance 
